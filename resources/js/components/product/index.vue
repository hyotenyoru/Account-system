      ...
      <table class="table mt-1">
        ...
        <tbody>
          <tr v-for="(item,start) in list" :key="start">
            ...
            <td>
              <a v-bind:href="index['url']+item['product_id']"><span  class="badge bg-primary">檢視</span></a>
              <a v-bind:href="index['url']+item['product_id']+'/edit'"><span  class="badge bg-secondary">修改</span></a>
            </td>
          </tr> 
        </tbody>
      </table>   
      ...
      computed:{
        list:function () {
          ...
          this.datas.forEach(array => {
            if (array[this.key].indexOf(this.search)!=-1) 
                list.push(array);
          });
          return list;
        }
